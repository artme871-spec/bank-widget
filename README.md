...
## Модуль generators
Содержит функции-генераторы для эффективной обработки транзакций:
* `filter_by_currency` — фильтрует операции по заданной валюте (USD, RUB и др.).
* `transaction_descriptions` — поочередно возвращает текстовые описания транзакций.
* `card_number_generator` — генерирует номера карт в формате `XXXX XXXX XXXX XXXX` в указанном диапазоне.
